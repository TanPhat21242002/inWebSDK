...
class PointDragger extends State {
  constructor(...args) {
    super(...args);
    ...
    const screenToWorld = (x, y) => {
      ...
      const pGsView = view
      point.transformBy((pGsView.screenMatrix().mul(pGsView.projectionMatrix())).inverse());
      point.z = 0.;

      const xEyeToWorld = new OdGeMatrix3d();
      let yVector = pGsView.upVector().normalize();
      let zVector = (pGsView.position().asVector().sub(pGsView.target().asVector())).normalize();
...
class CircleDragger extends State {
  constructor(...args) {
    super(...args);
...
class ArcDimensionDragger extends State {
